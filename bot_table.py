...
sys.stdout.flush()
print("=" * 60, flush=True)
print("🃏 ПРОГНОЗИСТ 2 - ПО РАНГУ (ИСПРАВЛЕННАЯ НУМЕРАЦИЯ)", flush=True)
print("=" * 60, flush=True)
...
def check_results(history, all_messages):
    """Проверяет результаты прогнозов — ИГРОК + ДИЛЕР, мгновенно"""
...
def main():
    ...
    print("🔄 ПРОГНОЗИСТ 2 (ПО РАНГУ) ЗАПУЩЕН", flush=True)
    ...
    print("   - Находим старший ранг (10>9>8>7>6>K>Q>J>A)", flush=True)
